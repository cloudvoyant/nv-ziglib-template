...
The project integrates Zig's native build system (build.zig) with a portable bash framework for CI/CD automation. Multi-platform binaries are published to GitHub Releases by default, with optional GCP Artifact Registry support for enterprise distribution.

## How It Works

The project integrates Zig with portable automation:
...
4. The justfile also calls bash scripts in `scripts/` for cross-language automation (`setup.sh`, `upversion.sh`)
...
After scaffolding from the template, you have a working Zig project ready to build. Interact with your project via `just` commands:
...
The project is configured for Zig 0.15.1:
...
The key principle: Zig handles all language-specific operations, justfile provides the interface, bash scripts handle cross-language concerns (versioning, setup). This separation keeps Zig idioms pure while maintaining portable automation.
...
This section describes how components work internally.

### Component: justfile

The `justfile` serves as the command runner interface, providing a consistent command experience. It uses bash as the shell interpreter and defines color variables (INFO, SUCCESS, WARN, ERROR) for pretty output. The `_load` recipe sources `.envrc` to load environment variables, and all other recipes depend on it to ensure consistent configuration.
...
The `scripts/` directory contains language-agnostic bash automation, isolating platform-specific complexity from your project code. This design allows consistent workflows across different languages.

- `setup.sh` - Installs dependencies using semantic flags (`--dev`, `--ci`) that clearly communicate intent and avoid installing unnecessary tools in CI environments
...
Keep it simple with just `export` statements - no bash logic. This constraint prevents complex logic from hiding in configuration, making projects easier to debug. Secrets belong in GitHub Secrets, not `.envrc`, following security best practices.
...
This approach makes dependencies explicit and context-aware. Developers get linting and formatting tools (`--dev`), CI environments install only what's needed for builds (`--ci`). This reduces CI build times, prevents tool version conflicts, and makes onboarding clearer ("run `just setup --dev` to get started").

### Publishing

The project publishes multi-platform Zig binaries to two destinations:
...
- Binary naming: `project-os-arch` (e.g., `{{PROJECT_NAME}}-linux-x86_64`)
...
The project works on macOS, Linux, and Windows (via WSL) without requiring users to install platform-specific tools. This broad compatibility reduces team onboarding friction and prevents "works on my machine" issues.
...
All scripts use `set -euo pipefail` for fail-fast behavior, ensuring errors don't silently propagate. Error traps handle cleanup on failure, preventing partial state. Lock files prevent concurrent script execution, avoiding race conditions during critical operations like version bumping.
